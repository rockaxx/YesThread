...
void sender_task(void* parameters)
{
    ...
    char message_buffer[8];  // stačí malé pole, 8 je s rezervou
    esp_err_t status;
    ...
    while (1) {
        message_counter++;
        if (message_counter > 9999) message_counter = 1; // reset po 9999
        
        // zapíš číslo ako string (napr. "1234")
        snprintf(message_buffer, sizeof(message_buffer), "%lu", (unsigned long)message_counter);
